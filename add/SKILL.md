...
Treat the scope as everything discussed in the full current chat, not just the latest message. In `/add push` mode, only include the chat-scoped work plus the minimum already-present supporting diff required for Railway to build successfully. Highly prefer commit slices of roughly 500 changed lines when the work can be separated cleanly. Treat any single pushed commit over 2,000 changed lines as a hard stop. If the work cannot be split below that ceiling safely, stop and tell the user instead of forcing a jumbo push.
...
- Highly prefer commit slices of roughly 500 changed lines when the work can be split cleanly.
...
- Determine whether the chat-scoped diff can be split into coherent commit slices of roughly 500 changed lines.
...
- Refuse `/add push` if any planned commit would exceed 2,000 changed lines and cannot be split safely.
...
- Never push a single commit over 2,000 changed lines.
...
4. Estimate the size of the chat-scoped diff and sketch commit slices that land around 500 changed lines when practical.
...
10. Before any `/add push` commit, verify the staged slice stays well-scoped, strongly prefer keeping it around 500 changed lines, and stop if it exceeds 2,000 changed lines.
11. Write one commit message per planned slice. Prefer the repository's existing style such as `feat(scope): ...`, `fix(scope): ...`, `refactor(scope): ...`, or `chore(scope): ...`.
12. If the invocation is plain `/add`, stop there. Report that the result was staged only, and include the suggested commit message. If a split would be better in `/add push` mode, say so explicitly.
13. If the invocation is `/add push` and one commit is planned, commit with that message.
14. If the invocation is `/add push` and multiple commits are planned, commit each staged slice separately in logical order, restaging between commits as needed.
15. In `/add push` mode, push explicitly from the checked out `main`, for example `git push origin main`.
16. If commit or push fails for any reason, stop and report the blocker clearly.
...
- When splitting into multiple commits, target roughly 500 changed lines per commit when practical.
- If a coherent slice must be larger than that, keep it comfortably below 2,000 changed lines.
- If a slice cannot be split safely below 2,000 changed lines, stop instead of forcing a jumbo push.
...
- `/add push` should strongly prefer commit slices around 500 changed lines.
- `/add push` must never push a commit over 2,000 changed lines.
...
`/add push` means: do the same surgical staging on `main`, include only the minimum extra diff required for a successful Railway build, prefer commit slices around 500 changed lines, never push a commit over 2,000 changed lines, then commit on `main` and push `main`.
