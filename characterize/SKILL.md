...
8. Let the closed tabs influence the split when they reflect genuinely separate streams of work, and treat them as strong candidate boundaries unless the code is clearly inseparable. If a change appears tied only to an active tab, leave it out of the characterized buckets and call out that it was intentionally left alone because it is still in flight.
9. For each bucket, provide:
   - a short label in kebab-case
   - the files that belong to it
   - a `1-4` sentence plain-English summary of what distinct feature, fix, or supporting change that commit includes, preferring `1-2` and staying concise
   - one conventional-commit style message with the approximate changed-line count on the same line
...
When there are honest split points, prefer commits in roughly the `150-500` changed-line range, and try to keep buckets around `500` changed lines or less when practical. Once a bucket drifts past about `500` changed lines, actively assume there may be a cleaner split by product surface, runtime versus UI, behavior versus presentation, or one closed-tab thread versus another. User-authored buckets above about `700` changed lines should be rare and need a stronger reason than convenience. In push mode, `1200` changed lines is the default maximum for a user-authored commit. Do not commit or push a bucket over `1200` changed lines unless it fits the narrow generated-file exception below.

Generated-file exception: a commit may exceed `1200` changed lines when the oversized portion is exactly one machine-managed file that is not user-authored code, such as `package-lock.json`, `pnpm-lock.yaml`, `yarn.lock`, `bun.lockb`, `poetry.lock`, `Cargo.lock`, a generated schema snapshot, or another generated artifact. Keep this exception tiny: the commit should contain only that generated file, or that generated file plus the smallest required companion manifest such as `package.json`. It must not include user-authored source, docs, tests, UI, migrations, or unrelated config changes. The commit message should name the generated/dependency artifact honestly.

If a bucket lands between `700` and `1200` changed lines, explicitly sanity-check whether it should be split before keeping it whole. If a commit has to exceed `1200` changed lines under the generated-file exception, its summary must explicitly say which generated file accounts for the size.
...
4. Run `git diff --cached --stat`, `git diff --cached --numstat`, and `git diff --cached --shortstat`. Confirm the staged files and changed-line count match one coherent bucket. If the staged diff is over `1200` changed lines and does not meet the generated-file exception above, stop and split the bucket before committing.
...
9. Confirm every commit still satisfies the `1200` line rule or generated-file exception, every title matches its diff, the working tree has no accidental leftovers, and the current branch still matches the branch you told the user you would push. If anything is wrong, fix the local commits before pushing.
