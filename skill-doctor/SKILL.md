...
- Treat the second argument after `/skill-doctor` as the name of an existing skill.
...
- Treat `/skill-doctor` as review-only.
- Never edit a skill during the initial `/skill-doctor` response.
- Treat the `/skill-doctor` invocation itself as insufficient consent to edit.
